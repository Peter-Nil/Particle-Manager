...
#include <OpenTissue\Core\Math\MathIsNumber.h>
    ...
    namespace math
    {
        ...
        public:
            ...
            */
            ...
            // Added by Christian Larsson
            vector3_type rotateBack(vector3_type const& r) const { return (conj(*this) % Quaternion(0, r) % (*this)).v(); }

            // Added by Christian Larsson
            void rotate(double yawRad, double pitchRad, double rollRad) // yaw (Z), pitch (Y), roll (X)
            {
                using std::cos;
                using std::sin;
                ...
                m_s     = cr * cp * cy + sr * sp * sy;
                m_v(0)  = sr * cp * cy - cr * sp * sy;
                m_v(1)  = cr * sp * cy + sr * cp * sy;
                m_v(2)  = cr * cp * sy - sr * sp * cy;
            }

            // Added by Christian Larsson
            void lookAt(const vector3_type& sourcePoint, const vector3_type& destPoint, const vector3_type& front, const vector3_type& up)
            {
                vector3_type toVector = normalize(destPoint - sourcePoint);

                //compute rotation axis
                vector3_type rotAxis = normalize(cross(front, toVector));
                if (sqr_length(rotAxis)/*squaredNorm()*/ == 0)
                    rotAxis = up;

                //find the angle around rotation axis
                float _dot = dot(/*VectorMath::front()*/vector3_type(0, 0, 1), toVector);
                float ang = std::acosf(_dot);

                //convert axis angle to quaternion
                Quaternion result = angleAxisf(rotAxis, ang);
                m_v[0]  = result.m_v[0];
                m_v[1]  = result.m_v[1];
                m_v[2]  = result.m_v[2];
                m_s     = result.m_s;
            ...
            Quaternion angleAxisf(const vector3_type& axis, float angle)
            {
                auto s = std::sinf(angle / 2);
                auto u = normalize(axis);
                return Quaternion(std::cosf(angle / 2), u.x * s, u.y * s, u.z * s);
            }

            void lookAt2(vector3_type& sourcePoint, vector3_type& destPoint)
            {
                vector3_type forwardVector = normalize(destPoint - sourcePoint);

                vector3_type rotAxis = cross(vector3_type(0, 0, 1), forwardVector);
                float _dot = dot(vector3_type(0, 0, 1), forwardVector);
            ...
            matrix3x3_type toMatrix() const
            {
                matrix3x3_type ret;
                ret[0][0] = 1 - 2  * m_v[1] * m_v[1] - 2 * m_v[2] * m_v[2];
                ret[1][0] = 2 * m_v[0] * m_v[1] + 2  * m_s * m_v[2];
                ret[2][0] = 2 * m_v[0] * m_v[2] - 2  * m_s * m_v[1];

                ret[0][2] = 2 * m_v[0] * m_v[1] - 2 * m_s * m_v[2];
                ret[1][2] = 1 - 2  * m_v[0] * m_v[0] - 2 * m_v[2] * m_v[2];
                ret[2][2] = 2 * m_v[1] * m_v[2] + 2 * m_s * m_v[0];

                ret[0][2] = 2 * m_v[0] * m_v[2] + 2 * m_s * m_v[1];
                ret[1][2] = 2 * m_v[1] * m_v[2] - 2 * m_s * m_v[0];
                ret[2][2] = 1 - 2 * m_v[0] * m_v[0] - 2 * m_v[1] * m_v[1];
                return ret;
            }
